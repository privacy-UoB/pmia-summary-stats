...
def run_experiment(iterations=2000):
    ...
    for disease in TARGET_DISEASES:
        ...
        for n in pool_sizes:
            ...
            # Save incrementally
            pd.DataFrame(results).to_csv("nsweep_results.csv", index=False)
...
def make_auc_figure(csv_path="nsweep_results.csv"):
    ...
    for ax, disease in zip(axes, TARGET_DISEASES):
        ...
        ax.plot(ns, ddf["case_auc_llr"].values, "b-o", ms=4, lw=1.2,
                label="Case LLR")
        ax.plot(ns, ddf["case_auc_l1"].values, "r-o", ms=4, lw=1.2,
                label="Case L1")
        ax.plot(ns, ddf["random_auc_llr"].values, "b--s", ms=4, lw=1.2,
                label="Random LLR")
        ax.plot(ns, ddf["random_auc_l1"].values, "r--s", ms=4, lw=1.2,
                label="Random L1")
